...
export function AboutSection() {
  return (
    <section className="border-y border-border bg-surface px-4 py-20 md:py-28">
        ...
        <div>
          ...
          <div className="mt-8 space-y-4">
            {timeline.items.map((item) => (
              <div
                key={item.year}
                className="grid grid-cols-[88px_1fr] rounded-2xl border border-border bg-surface-muted p-4"
              >
                <p className="font-black text-primary font-hanken">{item.year}</p>
                <p className="text-sm font-semibold leading-6 text-text">{item.text}</p>
              </div>
            ))}
          </div>
        </div>
